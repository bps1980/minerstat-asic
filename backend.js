        ...
        if (error != null) {
            ...
            if (json_string.indexOf("asic") > -1) {
                ...
                for (var id in obj) {
                    ...
                    console.log("[%s] List => %s worker %s {%s} ›%s/%s‹", getDateTime(), type, worker, ip_address, login, passw);
                    // SEND FOR ASYNC PROCESSING
                    workerProcess(accesskey, worker, ip_address, type, login, passw, remotecommand, isconfig);
                }
            } else {
                restartNode();
                console.log("[%s] ERROR => %s", getDateTime(), error);
...
async function fetchHTTP(worker, workerIP, workerType, httpLogin, httpPass) {
    request({
        ...
        headers: {
            "Authorization": "Basic " + new Buffer(login + ":" + passw).toString(ASIC_DEVICE[workerType].http_auth_type)
        }
    }, function(error, response, body) {
